...
Template.mealEdit.OnRendered(function () {
  if (Date.parseDate = function (e, t) {
...
Template.mealEdit.events({
  ...
  "click #meal-edit-button": function () {
    ...
    var o = {},
      s = [],
      l = $("#event-edit-auto-accept-toggle").is(":checked");
    o.autoAccept = l;
    ...
    var p = $(".meal-edit-form #meal-edit-start-time").val();
    r(p) ||
    ...
    if (m(D) ? (o.address = {}, o.address.full = D) :
      ...
      for (var E = [], T = 0; T < s.length; T++)
        E.push({ reason: s[T] });
      Session.set("Edit meal errors", E),
        Session.set("Edit meal error", true)
    } else
      Meteor.call("editMeal", Session.get("editMealId"), o,
        function (e, t) {
          if (e) {
            $(".pusher").dimmer("hide"), $("html, body").animate({ scrollTop: 0 },
